...
python utils/zero_to_fp32.py ${model}/checkpoint-${step}/ ${model}/tmp-checkpoint-${step}/pytorch_model.bin
cp ${model}/*.json ${model}/tmp-checkpoint-${step}
python utils/convert.py -s ${model}/tmp-checkpoint-${step} -t ${model}/final-checkpoint-${step}
